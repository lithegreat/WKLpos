...
一套专为美发用品专卖、商超零售店量身定制的现代化 Windows 原生 POS 收银与进销存系统。基于 **.NET 10** 构建，遵循 **清晰分层架构 (Clean Architecture)** 与 **MVVM 模式**，采用 **WinUI 3 (Windows App SDK 1.6)** 原生现代设计（Mica 材质与毫秒级瞬时响应）。当前版本：**v0.2.3.1**。
...
```
生成的安装包将存放在 `output_installer\WanKePos_Setup_v0.2.3.1.exe`（仅约 23.6 MB），脚本会自动完成编译发布、无用资源清洗、安装包制作，并在本机自动静默安装及拉起运行验证。
